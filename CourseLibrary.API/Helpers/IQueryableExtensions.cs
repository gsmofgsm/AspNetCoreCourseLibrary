    ...
    public static class IQueryableExtensions
    {
            ...
            Dictionary<string, PropertyMappingValue> mappingDictionary)
        {
            ...
            foreach (var orderByClause in orderByAfterSplit.Reverse())
            {
                ...
                var trimmedOrderByClauses = orderByClause.Trim();

                // if the sort option ends with " desc", we order
                // desceding, otherwise ascending
                var orderDescending = trimmedOrderByClauses.EndsWith(" desc");

                // remove " asc" or " desc" from the orderByClause, so we 
                // get the property name to look for in the mapping dictionary
                var indexOfFirstSpace = trimmedOrderByClauses.IndexOf(" ");
                var propertyName = indexOfFirstSpace == -1 ?
                    trimmedOrderByClauses : trimmedOrderByClauses.Remove(indexOfFirstSpace);
